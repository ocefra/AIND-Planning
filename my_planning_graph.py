...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # and add a PgNode_a to a_levels[0] set
        ...
        # Once an action node is added, it MUST be connected to the S node instances
    ...
    def add_literal_level(self, level):
        ...
        # for example, every A node in the previous level has a list of S nodes in effnodes
        # that represent the effect produced by the action.  These literals will all be part
        # of the new S level.  Since we are working with sets, they may be "added" to the set
        # without fear of duplication.  However, it is important to then correctly create
        # and connect all of the new S nodes as children of all the A nodes that could produce them,
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        def negates_effect(node_a1, node_a2) -> bool:
            for effect in node_a1.action.effect_add:
                if effect in node_a2.action.effect_rem:
                    return True

        return negates_effect(node_a1, node_a2) or negates_effect(node_a2, node_a1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        def negates(node_a1, node_a2, attribute1, attribute2):
            for effect in eval('node_a1.action.' + attribute1):
                if effect in eval('node_a2.action.' + attribute2):
                    return True

        def negates_precond(node_a1, node_a2):
            return negates(node_a1, node_a2, 'effect_add', 'precond_neg') or \
                negates(node_a1, node_a2, 'effect_rem', 'precond_pos')

        return negates_precond(node_a1, node_a2) or negates_precond(node_a2, node_a1)
